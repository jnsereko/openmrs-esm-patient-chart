...
export interface PatientDetailProps {
  ...
  stickerField: StickerFieldConfig;
}

export const PatientName: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
  const { t } = useTranslation();
  return (
    <div className={styles.fieldRow}>
      <span className={styles.patientDetailLabel}>{t(stickerField.label)}</span>
      <span className={styles.patientDetail}>{getPatientName(patient)}</span>
    </div>
  );
};

export const StickerBarcode: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
  const primaryIdentifierValue = patient?.identifier?.find((identifier) => identifier.use === 'official')?.value;

  return <Barcode value={primaryIdentifierValue} {...defaultBarcodeParams} />;
};

export const StickerLogo: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
  const { t } = useTranslation();

  return (
    <div className={styles.implementationLogo}>
      {stickerField?.logoURL ? (
        <img alt={t('implementationLogo', 'Implementation logo')} src={stickerField?.logoURL} />
...
export const PatientAge: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
  const { t } = useTranslation();
  return (
    <div className={styles.fieldRow}>
      <span className={styles.patientDetailLabel}>{t(stickerField.label)} </span>
...
export const PatientDob: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
  const { t } = useTranslation();
  return (
    <div className={styles.fieldRow}>
      <span className={styles.patientDetailLabel}>{t(stickerField.label)} </span>
...
export const PatientGender: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
  const { t } = useTranslation();
...
export const PatientIdentifier: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
...
export const PatientContact: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
  const { t } = useTranslation();
...
export const PatientAddress: React.FC<PatientDetailProps> = ({ patient, stickerField }) => {
